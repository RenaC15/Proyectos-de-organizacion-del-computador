...
ia_activa: .word 0          # 0 = inactiva, 1 = activa
ia_dificultad: .word 2      # 1 = fácil, 2 = medio, 3 = difícil
ia_contador: .word 0        # contador para retardar movimientos IA
...
modo_2_jugadores:
    li $t0, 2
    sw $t0, modo_juego
    sw $zero, ia_activa        # Desactivar IA
    j iniciar_juego
...
# =============================================
# NUEVA FUNCIÓN: IA_MOVER_PALETA_DERECHA
# =============================================
ia_mover_paleta_derecha:
    addi $sp, $sp, -16
    sw $ra, 12($sp)
    sw $s0, 8($sp)
    sw $s1, 4($sp)
    ...
    # Solo mover cada N frames según dificultad
    lw $t1, ia_dificultad
    li $t2, 1
    beq $t1, $t2, ia_facil_check   # Fácil: mover menos
    li $t2, 2
    beq $t1, $t2, ia_medio_check   # Medio: mover normal
    j ia_dificil_check              # Difícil: mover más
    
ia_facil_check:
    andi $t3, $t0, 0x3             # Mover cada 4 frames
    bnez $t3, ia_fin
    j ia_decidir_movimiento
    
ia_medio_check:
    andi $t3, $t0, 0x1             # Mover cada 2 frames
    bnez $t3, ia_fin
    j ia_decidir_movimiento
    
ia_dificil_check:
    # Mover cada frame (siempre)
    j ia_decidir_movimiento

ia_decidir_movimiento:
    # Obtener posición de la pelota y de la paleta derecha
    lw $t0, pelota_fila
    lw $t1, posJugD
    
    # Calcular diferencia
    sub $t2, $t0, $t1  # diferencia = pelota_fila - posJugD
    
    # Determinar si la pelota viene hacia la IA
    lw $t3, pelota_dir_col
    blez $t3, ia_fin  # Si la pelota va hacia la izquierda, no mover
    
    # Ajustar según dificultad
    lw $t4, ia_dificultad
    li $t5, 1
    beq $t4, $t5, ia_facil_logic
    li $t5, 2
    beq $t4, $t5, ia_medio_logic
    j ia_dificil_logic

ia_facil_logic:
    # Fácil: solo mueve si la diferencia es grande
    li $t6, 2
    abs $t7, $t2
    blt $t7, $t6, ia_fin
    j ia_aplicar_movimiento

ia_medio_logic:
    # Medio: mueve si hay diferencia
    beqz $t2, ia_fin
    j ia_aplicar_movimiento

ia_dificil_logic:
    # Difícil: siempre intenta alinearse
    j ia_aplicar_movimiento

ia_aplicar_movimiento:
    # Decidir dirección
    beqz $t2, ia_fin        # No mover si están alineados
    bgtz $t2, ia_mover_abajo
...
main_loop:
    ...
    # MOVIMIENTO JUGADOR DERECHO: Solo permitir en modo 2 jugadores
    lw $t3, modo_juego
    li $t4, 2
    bne $t3, $t4, skip_derecha_controls
...
skip_derecha_controls:
    # Servicio
    li $t2, 'x'
    beq $t1, $t2, call_servir1
    
    # Servicio jugador 2 solo en modo 2 jugadores
    lw $t3, modo_juego
    li $t4, 2
    bne $t3, $t4, skip_input
    li $t2, 'm'
